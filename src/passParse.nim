...
proc mkAstMain*[T: HaveToAstNode](
  ...
  #kind: AstKind,
  obj: T,
  lexMain: Option[LexMain]=none(LexMain)
): AstNode = 
  #result = AstNode(
  ...
  #  #obj
  #)
  var tempLexMain: LexMain
  if lexMain.isSome:
    tempLexMain = lexMain.get()
  else:
    tempLexMain = self.lexMain
  result = toAstNode(
    obj=obj,
    lexMain=(
      #self.lexMain
...
proc parseTypeBasicBuiltin(
  ...
  result.ast = mkAst(
    #astTypeSub
    AstTypeSub(kind: typeSubBasicType)
  )
  #result.ast.myTypeSub.kind = typeSubNamedType
  #result.ast.myTypeSub.myBasicType = SubAstB
  case result.foundTok.get():
  of tokU8:
    result.ast.myTypeSub.myBasicType.kind = basicTypeU8
  of tokU16:
    result.ast.myTypeSub.myBasicType.kind = basicTypeU16
  of tokU32:
    result.ast.myTypeSub.myBasicType.kind = basicTypeU32
  of tokU64:
    result.ast.myTypeSub.myBasicType.kind = basicTypeU64
  of tokI8:
    result.ast.myTypeSub.myBasicType.kind = basicTypeI8
  of tokI16:
    result.ast.myTypeSub.myBasicType.kind = basicTypeI16
  of tokI32:
    result.ast.myTypeSub.myBasicType.kind = basicTypeI32
  of tokI64:
    result.ast.myTypeSub.myBasicType.kind = basicTypeI64
  of tokF32:
    result.ast.myTypeSub.myBasicType.kind = basicTypeF32
  of tokF64:
    result.ast.myTypeSub.myBasicType.kind = basicTypeF64
  of tokString:
    result.ast.myTypeSub.myBasicType.kind = basicTypeString
  of tokChar:
    result.ast.myTypeSub.myBasicType.kind = basicTypeChar
  of tokVoid:
    result.ast.myTypeSub.myBasicType.kind = basicTypeVoid
  else:
    doAssert(
      false,
      "eek! " & $result
...
proc parseTypeToResolve(
  ...
  #echo sconcat(@[
  #  "testificate: ",
  #  $result.ast.myTypeSub[], " ",
  #  $(result.ast.myTypeSub.myNamedType == nil)
  #])
  result.ast.myTypeSub.myNamedType.ident = myIdent.myIdent
  ...
  #discard self.selParse(chk=false, @[spp parseGenericFullImplList])
  #if mySpp.foundSelProc.isSome:
  #  discard mySpp.foundSelProc.get()(self=self, chk=false)

  discard self.optParse(chk=false, selProc=spp parseGenericFullImplList)
  #if temp.foundTok.isSome:
  #  result.ast.myTypeSub.myNamedType.genericImpl = temp.ast
  #else:
  #  result.ast.myTypeSub.myNamedType.genericImpl = mkAst(astGenericList)
...
proc parseTypeBuiltinWithoutOptPreKwVar(
  ...
  result.ast.myType.child = (
    self.parseTypeMainBuiltin(chk=false).ast.myTypeSub
  )
...
proc parseTypeWithOptPreKwVar(
  ...
  if haveEither:
    if ptrDim == 0:
      ...
      result.ast.myType.ptrDim = ptrDim
  #echo $result.ast[]
  #echo $result.ast.myType[]
  let temp = self.parseTypeMain(chk=false).ast#.myTypeSub
  #echo $temp[]
  result.ast.myType.child = (
    temp.myTypeSub
  )
...
proc parseTypeWithoutOptPreKwVar(
  ...
  if haveEither:
    if ptrDim == 0:
      ...
      result.ast.myType.ptrDim = ptrDim
  result.ast.myType.child = (
    self.parseTypeMain(chk=false).ast.myTypeSub
  )
  #discard self.optParse(chk=false, selProc=spp parseTypeArrDim)
...
proc parseVarEtcDeclMost(
  ...
  result = doChkSpp(parseIdent)
  var myIdent = result.ast.myIdent
  result.ast = mkAst(
    #astVarEtcDeclMost
    AstVarEtcDeclMost()
  )
  ...
  if chk:
    ...
    if result.foundTok.isSome:
      ...
      of astDef:
        ...
        #  result.ast
        #)
        parent.myDef.genericDeclSeq.add(
          result.ast.myIdent
        )
      of astStruct:
        ...
        #  result.ast
        #)
        parent.myStruct.genericDeclSeq.add(
          result.ast.myIdent
        )
      else:
        doAssert(
...
proc parseGenericNamedImplItem(
  ...
  if not chk:
    var tempType = self.parseTypeWithoutOptPreKwVar(chk=false).ast.myType
    var myIdent = result.ast.myIdent
    ...
    var genericImplSeq: ptr seq[AstGenericArgImpl] = nil
    case parent.kind:
    of astTypeSub:
      genericImplSeq = addr parent.myTypeSub.myNamedType.genericImplSeq
    of astExpr:
      genericImplSeq = addr parent.myExpr.myFuncCall.genericImplSeq
    else:
      doAssert(
        false,
        "eek! " & $parent.kind
...
proc parseGenericUnnamedImplItem(
  ...
  if chk:
    ...
    # I think that for this specific parse rule we *DO NOT* need to make
    # the `result` has 
    genericImplSeq[].add(
      AstGenericArgImpl(
        ident: none(AstIdent),
        type: result.ast.myType,
      )
    )
    result.ast = genericImplSeq[][^1].toAstNode()
    #of astStruct:
    #  genericImplSeq = addr self.parentTempSeq[^1].
...
proc subParseFuncArgDeclList(
  ...
  self.lexAndExpect(tokColon)
  result.ast.myVarEtcDeclMost.type = (
    self.parseTypeWithOptPreKwVar(chk=false).ast.myType
  )
  self.parentTempSeq[^1].myDef.argDeclSeq.add(
    result.ast.myVarEtcDeclMost
  )
...
proc parseFuncNamedArgImplItem(
  ...
  if not chk:
    #discard self.parseTypeWithOptPreKwVar(chk=false)
    var tempExpr = self.parseExpr(chk=false).ast.myExpr
    var myIdent = result.ast.myIdent
    ...
    var myExpr = self.parentTempSeq[^1].myExpr
    myExpr.myFuncCall.argImplSeq.add(
      result.ast.myFuncArgImpl
    )
  #discard self.parseType(chk=false)
...
proc parseFuncUnnamedArgImplItem(
  ...
  if chk:
    ...
    case tempAst.kind:
    of astExpr:
      case tempAst.myExpr.kind:
      of exprOpenarrLit:
        tempAst.myExpr.myOpenarrLit.openarrLitSeq.add(
          result.ast.myExpr
        )
      of exprFuncCall:
        result.ast = mkAst(
          AstFuncArgImpl(
            ident: none(AstIdent),
            expr: result.ast.myExpr
          )
        )
        tempAst.myExpr.myFuncCall.argImplSeq.add(
          result.ast.myFuncArgImpl
        )
      else:
        doAssert(
          false,
          "eek (expr)! " & $tempAst.myExpr.kind & " " & $self.lexMain
        )
    else:
      doAssert(
        false,
...
proc parseFuncDecl(
  ...
  result.ast.myDef.ident = self.parseIdent(chk=false).ast.myIdent
  ...
  self.lexAndExpect(tokSemicolon)
  result.ast = self.parentTempSeq.pop()
  self.astRoot.mySrcFile.funcDeclSeq.add(
    result.ast.myDef
  )
...
proc subParseStructDecl(
  ...
  self.parentTempSeq[^1].myStruct.fieldSeq.add(
    result.ast.myVarEtcDeclMost
  )
...
proc parseStructDecl(
  ...
  result.ast.myStruct.ident = self.parseIdent(chk=false).ast.myIdent
  ...
  self.lexAndExpect(tokSemicolon)
  result.ast = self.parentTempSeq.pop()
  self.astRoot.mySrcFile.structDeclSeq.add result.ast.myStruct
...
proc parseModule(
  ...
  tempModule.ident = (
    self.parseIdent(chk=false).ast.myIdent
  )
...
proc parseExprFuncCallPostIdent(
  ...
  if chk:
    ...
    if result.foundTok.isSome or temp.foundTok.isSome:
      ...
      #var parent = addr self.parentTempSeq[^1]
      #if result.foundTok.isSome:
      #  parent.myFuncCall.genericImpl = result.ast
      #else:
      #  parent.myFuncCall.genericImpl = (
...
proc parseExprIdentOrFuncCall(
  ...
  proc subParseExprIdentOrFuncCall(
    ...
    if temp.foundTok.isSome:
      let myIdent = result.ast.myIdent
      result.ast = self.parentTempSeq[^1]
      result.ast.myExpr.myFuncCall.ident = myIdent
    else:
      result.ast = mkAst(
        AstExpr(
          kind: exprExprIdent,
        ...
        lexMain=some(result.ast.lexMain)
      )
  ...
  proc subParseExprBuiltinTypeCast(
    ...
    self.lexAndExpect(tokLParen)
    var myType = result.ast.myType
    result.ast = mkAst(
      #astBuiltinTypeCast
      AstExpr(
        kind: exprBuiltinTypeCast,
      ),
      lexMain=some(result.ast.lexMain),
    )
    result.ast.myExpr.myBuiltinTypeCast.type = myType
    result.ast.myExpr.myBuiltinTypeCast.obj = (
...
proc parseU64Lit(
  ...
  #defer: discard unstack()
  result.ast.myExpr.myU64Lit.u64Val = self.currTok.optU64.get()
  ...
  if chk:
    ...
    if result.foundTok.isSome:
      ...
      while true:
        ...
        result.ast.myExpr.myBinop.kind = (
          tokToBinop(result.foundTok.get()).get()
        )
        result.ast.myExpr.myBinop.left = someSppRet.ast.myExpr
        #defer: discard unstack()
        result.ast.myExpr.myBinop.right = (
          subExprParseFunc(self=self, chk=false).ast.myExpr
        )
...
proc parseExprSuffixFieldMethodAccessDotExpr(
  ...
  of astIdent:
    ...
    result.ast.myExpr.myDot.right = temp.ast.myIdent
  of astExpr:#astFuncCall:
    case temp.ast.myExpr.kind:
    of exprFuncCall:
      result.ast = temp.ast
    else:
      doAssert(
...
proc parseExprSuffixDeref(
  ...
  result.ast.myExpr.myDeref.obj = self.parentTempSeq.pop().myExpr
  self.parentTempSeq.add result.ast
  ...
  if chk:
    ...
    if temp.foundSelProc.get() == parseExprSuffixFieldMethodAccess:
      ...
      of astExpr:
        ...
        #  discard
        of exprDot:
          var myPrev = self.parentTempSeq.pop().myExpr
          myTemp.ast.myExpr.myDot.left = myPrev
          self.parentTempSeq.add myTemp.ast
        else:
          doAssert(
            false,
  ...
  if chk:
    ...
    if result.foundTok.isSome or temp.foundTok.isSome:
      if result.foundTok.isSome:
        ...
        result.ast.myExpr.myUnop.kind = (
          tokToUnop(result.foundTok.get()).get()
        )
        result.ast.myExpr.myUnop.obj = (
          self.parseExprFieldArrEtc(chk=false).ast.myExpr
        )
      else:
...
proc parseStmtFor(
  ...
  result.ast.myStmt.myFor.ident = self.parseIdent(chk=false).ast.myIdent
  self.lexAndExpect(tokIn)
  result.ast.myStmt.myFor.exprPre = self.parseExpr(chk=false).ast.myExpr
  self.lex()
  if self.currTok.tok == tokTo:
    result.ast.myStmt.myFor.isUntil = false
  elif self.currTok.tok == tokUntil:
    result.ast.myStmt.myFor.isUntil = true
  else:
    self.expect(toHashSet([tokTo, tokUntil]))
  result.ast.myStmt.myFor.exprPost = self.parseExpr(chk=false).ast.myExpr
  self.lexAndExpect(tokLBrace)
  self.subParseStmtList(result.ast.myStmt.myFor.stmtSeq)
  self.lexAndExpect(tokRBrace)
proc parseStmtWhile(
  self: var Scone,
  chk: bool,
  ...
  result.ast.myStmt.myWhile.expr = self.parseExpr(chk=false).ast.myExpr
  self.lexAndExpect(tokLBrace)
  self.subParseStmtList(result.ast.myStmt.myWhile.stmtSeq)
  self.lexAndExpect(tokRBrace)
...
proc parseStmtElif(
  ...
  result.ast.myElif.expr = self.parseExpr(chk=false).ast.myExpr
  self.lexAndExpect(tokLBrace)
...
proc parseStmtIf(
  ...
  result.ast.myStmt.myIf.expr = self.parseExpr(chk=false).ast.myExpr
  self.lexAndExpect(tokLBrace)
  self.subParseStmtList(result.ast.myStmt.myIf.stmtSeq)
  self.lexAndExpect(tokRBrace)
  var myElif = self.parseStmtElif(chk=true)
  while myElif.foundTok.isSome:
    myElif = self.parseStmtElif(chk=false)
    result.ast.myStmt.myIf.elifSeq.add myElif.ast.myElif
    myElif = self.parseStmtElif(chk=true)
  result.ast.myStmt.myIf.optElse = none(AstElse)
  var myElse = self.parseStmtElse(chk=true)
  if myElse.foundTok.isSome:
    result.ast.myStmt.myIf.optElse = (
      some(self.parseStmtElse(chk=false).ast.myElse)
    )
...
proc parseStmtCase(
  ...
  result.ast.myCase.expr = self.parseExpr(chk=false).ast.myExpr
  self.lexAndExpect(tokLBrace)
...
proc parseStmtSwitch(
  ...
  result.ast.myStmt.mySwitch.expr = self.parseExpr(chk=false).ast.myExpr
  self.lexAndExpect(tokLBrace)

  var myCase = self.parseStmtCase(chk=true)
  while myCase.foundTok.isSome:
    myCase = self.parseStmtCase(chk=false)
    result.ast.myStmt.mySwitch.caseSeq.add myCase.ast.myCase
    myCase = self.parseStmtCase(chk=true)

  result.ast.myStmt.mySwitch.optDefault = none(AstDefault)
  var myDefault = self.parseStmtDefault(chk=true)
  if myDefault.foundTok.isSome:
    result.ast.myStmt.mySwitch.optDefault = (
      some(self.parseStmtDefault(chk=false).ast.myDefault)
    )
  self.lexAndExpect(tokRBrace)
...
proc parseStmtScope(
  ...
  self.lexAndExpect(tokLBrace)
  self.subParseStmtList(result.ast.myStmt.myScope.stmtSeq)
  self.lexAndExpect(tokRBrace)
...
proc parseStmtReturn(
  ...
  result.ast.myStmt.myReturn.optExpr = none(AstExpr)
  var myExpr = self.parseExpr(chk=true)
  if myExpr.foundTok.isSome:
    result.ast.myStmt.myReturn.optExpr = (
      some(self.parseExpr(chk=false).ast.myExpr)
    )
  self.lexAndExpect(tokSemicolon)
  ...
  if chk:
    ...
    if result.foundTok.isSome or temp.foundTok.isSome:
      if result.foundTok.isSome:
        ...
        result.ast.myExpr.myUnop.kind = unopAddr
        result.ast.myExpr.myUnop.obj = temp.foundSelProc.get()(
          self=self,
          chk=false,
        ).ast.myExpr
      else:
...
proc parseStmtCallOrAssignEtc(
  ...
  if temp.foundTok.isSome:
    var myExprLhs = result.ast.myExpr
    ...
    result.ast.myStmt.myAssignEtc.kind = (
      tokToAssignEtc(temp.foundTok.get()).get()
    )
    result.ast.myStmt.myAssignEtc.left = myExprLhs
    result.ast.myStmt.myAssignEtc.right = (
      self.parseExpr(chk=false).ast.myExpr
    )
  else:
    var myExprLhs = result.ast.myExpr
    result.ast = mkAst(
      #astStmtExprLhs
      AstStmt(
        kind: stmtStmtExprLhs,
      )
    )
    result.ast.myStmt.myStmtExprLhs.expr = myExprLhs
  self.lexAndExpect(tokSemicolon)
